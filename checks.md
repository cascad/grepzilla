...
Что дальше по E?
e2e: ingest_visibility_hot (сразу после /ingest поиск без segments/shards возвращает свежие документы).
e2e: ingest_dedup_ext_id (вставить документ с тем же _id → в выдаче один hit благодаря дедупу).
мелочь: вернуть из /ingest поле hot_added (мы уже добавили) и проверить его тестом.


---------


🔹 Полезные доработки (после основных)
E-Hardening
WAL durability: опциональный fsync (env GZ_WAL_FSYNC=1) → File::sync_data в wal.append_batch.
Compactor: бэкграунд периодический sweep WAL → segment; батчи по N строк.
HotMem TTL: параметризуем в BrokerConfig (секунды) + метрика current size.
Backpressure: при переполнении HotMem — 429 Too Many Requests / manifest_error.

Пагинация & Курсоры
Обновить Paginator: кроме last_docid, класть remaining_candidates на сегмент (для UX прогресса).
В SearchCoordinator добавить поддержку page.cursor строго по seg-path + hot-bucket __hot__.

Наблюдаемость
Трассировка этапов (span’ы prefilter/verify/prefetch) + поля запроса (wildcard hash).
Прометеевые метрики (опционально): histogram по временам и размерам.
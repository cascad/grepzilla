...
// NEW: сериализация доступа к process-wide переменным окружения
use std::sync::{Mutex, OnceLock};

mod helpers;
use helpers::make_router_with_parallelism;

// NEW: глобальная блокировка для модификации env в тестах
...
async fn ingest_appends_segment_to_manifest_for_single_shard() {
    // NEW: сериализуем доступ к переменным окружения на время теста
    let _guard = ENV_LOCK.get_or_init(|| Mutex::new(())).lock().unwrap();

    // 1) укажем переменные окружения для публикации (до создания Router!)
    ...
    // NEW: Router поднимаем после установки env
...
async fn second_ingest_increments_generation() {
    ...
    let req1 = Request::builder()
        ...
        .uri("/ingest")
        .header("content-type", "application/json")
        .body(Body::from(docs1.to_string()))
        .unwrap();
    ...
    let req2 = Request::builder()
        ...
        .uri("/ingest")
        .header("content-type", "application/json")
        .body(Body::from(docs2.to_string()))
        .unwrap();
